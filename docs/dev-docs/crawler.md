...
```bash
python -m newsqa_rag.crawler.crawl_articles \
...
python -m newsqa_rag.crawler.crawl_articles --storage hf-bucket

# Local HTML only
python -m newsqa_rag.crawler.crawl_articles --storage filesystem

# Upload to the bucket and retain local ingestion-ready HTML
python -m newsqa_rag.crawler.crawl_articles --storage both
```

...
def portable_relpath(path: str | Path, root: str | Path) -> str:
    """Return a relative path when possible, otherwise an absolute path.

    ``os.path.relpath`` raises on Windows when ``path`` and ``root`` are on
    different drives. Evaluation tests and external artifact directories can
    legitimately cross that boundary.
    """

    resolved = Path(path).resolve()
    try:
        return os.path.relpath(resolved, Path(root).resolve())
    except ValueError:
        return str(resolved)
...
def artifact_record(path: str | Path, root: str | Path | None = None) -> dict:
    artifact = Path(path)
    relative = portable_relpath(artifact, root) if root else str(artifact)
